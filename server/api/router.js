...
const getPagePreview = require("./preview");
...
router.get("/preview/:link", async(req, res, next) => {
    try {
        console.log("getting it for ", req.params.link);
        const data = await getPagePreview(req.params.link);
...
router.get("/grab/:domain", (req, res, next) => {
    const options = {
        normalizeUrl: req.query["normalize-url"] === void 0 ?
            true :
            req.query["normalize-url"] === "true",
    };
